...
def plot_avg_base_experience(
    avg_base_experience: pd.Series,
    PATH=f"{pathlib.Path(__file__).parent.parent}/results",
) -> None:
    avg_base_experience.to_csv(f"{PATH}/csv/avg_base_experience.csv")
    ...
    plt.tight_layout()
    plt.savefig(f"{PATH}/visual/avg_base_experience.png")
    plt.close()


def plot_unique_abilities_count(
    unique_abilities_count: pd.Series,
    PATH=f"{pathlib.Path(__file__).parent.parent}/results",
) -> None:
    unique_abilities_count.to_csv(f"{PATH}/csv/unique_abilities_count.csv")
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"{PATH}/visual/unique_abilities_count.png")
    plt.close()


def plot_pokemon_group_counts(
    grouped_df: pd.DataFrame, PATH=f"{pathlib.Path(__file__).parent.parent}/results"
) -> None:
    grouped_df["type_ability"] = grouped_df["types"] + " & " + grouped_df["abilities"]

    grouped_df.to_csv(f"{PATH}/csv/pokemon_group_counts.csv", index=False)
    ...
    fig.write_html(f"{PATH}/visual/pokemon_group_counts.html")
